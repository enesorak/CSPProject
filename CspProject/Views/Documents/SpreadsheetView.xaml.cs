...
using CspProject.Services.Document;
            ...
            try
            {
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        throw new InvalidOperationException("Failed to load FMEA template into spreadsheet control", ex);
                    }
 ...
 public async Task LoadDocument(int documentId)
{
    ...
    try
    {
        ...
        if (_currentDocument?.Content != null)
        {
            ...
            await Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    using var ms = new MemoryStream(_currentDocument.Content);
                    spreadsheetControl.LoadDocument(ms, DevExpress.Spreadsheet.DocumentFormat.Xlsx);
                    spreadsheetControl.Modified = false;
                }
                catch (Exception ex)
                {
                    SentrySdk.CaptureException(ex);
                    throw new InvalidOperationException($"Failed to load document {documentId} into spreadsheet control", ex);
                }
            }, System.Windows.Threading.DispatcherPriority.Normal);
        ...
        {
            // Document bulundu ama content yok
            await ShowNotification("Warning", "Document found but has no content.");
        }
        else
        {
            // Document bulunamadı
            await ShowNotification("Error", $"Document with ID {documentId} not found.");
        }
    ...
    catch (Exception ex)
    {
        ...
        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                await PerformSaveAsync(showSuccessNotification: true);
            }
            catch (Exception ex)
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Error", $"Save failed: {ex.Message}");
            }
        }
        ...
        private async void RenameButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Rename Error", $"Failed to rename document: {ex.Message}");
            }
        }
        ...
        private async void ExportButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Export Error", $"Failed to export document: {ex.Message}");
            }
        }
 ...
 private async void SubmitButton_Click(object sender, RoutedEventArgs e)
{
    ...
    try
    {
        transaction = SentrySdk.StartTransaction("submit-for-review", "workflow");
        ...
        if (_currentDocument == null || _currentUser == null)
        {
            ...
            await ShowNotification("Error", "Document or user information is missing.");
            return;
        }
        ...
        {
            transaction.Finish(SpanStatus.InvalidArgument);
            await ShowNotification("Error", "No approver selected.");
            return;
        }
    ...
    catch (Exception ex)
    {
        ...
        await ShowNotification("Submission Failed",
        ...
        private async void ApproveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_currentDocument == null || _currentUser == null || DbContext == null)
                {
                    await ShowNotification("Error", "Document or user information is missing.");
                    return;
                }
            ...
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Approval Error", $"Failed to approve document: {ex.Message}");
            }
        }
        ...
        private async void RejectButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_currentDocument == null || DbContext == null)
                {
                    await ShowNotification("Error", "Document information is missing.");
                    return;
                }
            ...
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Rejection Error", $"Failed to reject document: {ex.Message}");
            }
        }
        ...
        private async void CheckApprovalsButton_Click(object sender, DevExpress.Xpf.Bars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                await ShowNotification("Check Complete", result);
            }
            catch (Exception ex)
            {
                SentrySdk.CaptureException(ex);
        ...
        private async void DocumentChangeLogButton_Click(object sender, DevExpress.Xpf.Bars.ItemClickEventArgs e)
        {
            ...
            {
                SentrySdk.CaptureException(ex);
                await ShowNotification("Error", $"Failed to load change history: {ex.Message}");
            }
        }
 ...
 public async Task LoadTemplateFromFile(string filePath)
{
    ...
    try
    {
        ...
        // ✅ UI thread'de yükle, stream'i düzgün dispose et
        await Dispatcher.InvokeAsync(() =>
        {
            try
            {
            ...
            {
                SentrySdk.CaptureException(ex);
                throw new InvalidOperationException($"Failed to load template from file: {filePath}", ex);
            }
        }, System.Windows.Threading.DispatcherPriority.Normal);
   ...
   public async Task CreateBlankDocument()
{
    ...
    try
    {
        ...
        // ✅ Byte array olarak oluştur, stream'i background thread'de kapat
        byte[] templateBytes = await Task.Run(() =>
        {
            var prevCulture = Thread.CurrentThread.CurrentCulture;
            try
            {
                ...
                {
                    workbook.SaveDocument(ms, DevExpress.Spreadsheet.DocumentFormat.Xlsx);
                    return ms.ToArray(); // ✅ Stream dispose edilmeden önce byte array'e çevir
                }
            }
            finally
            {
        ...
        // ✅ UI thread'de yeni stream ile yükle
        await Dispatcher.InvokeAsync(() =>
        {
            try
            {
            ...
            {
                SentrySdk.CaptureException(ex);
                throw new InvalidOperationException("Failed to load blank template into spreadsheet control", ex);
            }
        }, System.Windows.Threading.DispatcherPriority.Normal);
...
private async void TemplateInfoButton_Click(object sender, DevExpress.Xpf.Bars.ItemClickEventArgs e)
{
    ...
    {
        SentrySdk.CaptureException(ex);
        await ShowNotification("Error", $"Failed to get template info: {ex.Message}");
    }
}
