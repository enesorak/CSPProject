...
namespace CspProject
{
    public partial class MainWindow : ThemedWindow, INotifyPropertyChanged
    {
        ...
        {
            // ✅ Eski HomeScreen varsa dispose et
            if (_currentHomeScreen != null)
            {
                // HomeScreen IDisposable değil ama UserControl, içindeki resources temizlensin
                if (_currentHomeScreen is IDisposable disposable)
                {
                    disposable.Dispose();
                }
                _currentHomeScreen = null;
            }
        ...
        private async void ShowSpreadsheetScreen(int? documentId)
        {
            ...
            await Dispatcher.InvokeAsync(async () =>
            {
                try
                {
                    ...
                    spreadsheetView.EnsureInitialized();
        ...
        protected override void OnClosed(EventArgs e)
        {
            ...
            // ✅ HomeScreen'i temizle
            if (_currentHomeScreen != null)
            {
                // Unloaded event tetiklenir ama yine de manuel temizle
                if (MainContentControl.Content == _currentHomeScreen)
                {
